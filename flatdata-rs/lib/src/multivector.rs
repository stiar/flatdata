...
///
/// archive X {
///    heterogeneous_data : multivector< 16, A, B >;
/// }
/// ```
///
/// ```rust
...
/// #     test_archive::{X, XBuilder, HeterogeneousDataRef}
/// # };
/// #
/// // create multivector and serialize some data
/// let mut storage = MemoryResourceStorage::new("/root/multivec");
/// let mut builder = XBuilder::new(storage.clone()).expect("failed to create builder");
/// # let _ = builder.start_data().expect("failed to open extenal vector").close();
///
/// let mut mv = builder.start_heterogeneous_data().expect("failed to create MultiVector");
...
/// let mv = archive.heterogeneous_data();
///
/// assert_eq!(mv.len(), 1);
/// let mut item = mv.at(0);
/// match item.next().unwrap() {
///     HeterogeneousDataRef::A(a) => assert_eq!((a.x(), a.y()), (1, 2)),
///     _ => assert!(false),
/// }
/// match item.next().unwrap() {
///     HeterogeneousDataRef::B(b) => assert_eq!(b.value(), 42),
///     _ => assert!(false),
/// }
/// # }
/// ```
...
mod tests {
    use crate::{
        ...
        test_archive::{HeterogeneousData, HeterogeneousDataRef},
    };
    ...
    fn test_multi_vector() {
        ...
        {
            let mut mv =
                create_multi_vector::<HeterogeneousData>(&*storage, "multivector", "Some schema")
        ...
        let index: ArrayView<_> = ArrayView::new(&index_resource);
        let resource = storage
            .read_and_check_schema("multivector", "Some schema")
            .expect("read_and_check_schema failed");
        let mv: MultiArrayView<HeterogeneousData> = MultiArrayView::new(index, &resource);
