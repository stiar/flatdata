...
mod test {
    ...
    fn create_values(size: usize) -> Vector<A> {
        let mut v: Vector<A> = Vector::with_len(size);
    ...
    fn reverse() {
        ...
        let data: Vec<_> = iter.map(|value| value.x()).collect();
    ...
    fn test_slice() {
        ...
        assert_eq!(view.slice(2..).iter().next().unwrap().x(), 2);
        assert_eq!(view.slice(..8).len(), 8);
        assert_eq!(view.slice(..8).iter().next().unwrap().x(), 0);
        assert_eq!(view.slice(2..8).len(), 6);
        assert_eq!(view.slice(2..8).iter().next().unwrap().x(), 2);
    }
    ...
    fn debug() {
        ...
        assert_eq!(
            format!("{:?}", iter),
            "ArrayViewIter { len: 1, data: [A { x: 99, y: 99 }] }"
        );
    }
}
